function [g2g,Duration_off,Count_off,Rate_off,Duration_on,Count_on,Rate_on,NonREMdur_tot,NPD]=get_off_data_trial_pt5(SD_Data_Veh_Pyr,i,ii,NPD,Trial)
 ...
 NonREMdur_tot=[];
  ...
  jk=1;
 if isempty(x(jk).('Post_Trial_5_All'))
     jk=2;
 end
 uplim=length(x(jk).('Post_Trial_5_All').(Trial).NREM_Spike_Times);
         parfor iii=1:uplim % NREM bouts. 
             X=[]; % compressed data (mua) per bout
             Y=[];
             for iiii=1:length(xxx) % rows/neurons. (Some rows are empty)
               if isempty(x(iiii).('Post_Trial_5_All'))
                    continue
                end
                 
                X=[X; x(iiii).('Post_Trial_5_All').(Trial).NREM_Spike_Times{iii,1}];
                y1=x(iiii).('Post_Trial_5_All').(Trial).NREM_Bout_Info{iii,1};
                y2=x(iiii).('Post_Trial_5_All').(Trial).NREM_Bout_Info{iii,2};
